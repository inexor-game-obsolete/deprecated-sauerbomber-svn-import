...
from core.colors import blue, red, orange, colordict
from core.ui import error, info
from core.settings import PluginConfig
from players import isMaster, isAdmin
import logging
...
def onPlayerCommands(cn, args):
	if args != '':
		sauerbomber.playerMessage(cn, error('Usage: #playercommands'))
	else:
		msg = blue('Available commands: ')
		for command in command_info.keys():
			msg += '#' + command + ' '
		sauerbomber.playerMessage(cn, orange(msg))
