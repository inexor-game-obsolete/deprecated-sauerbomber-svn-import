...
//Hanack    int gamemode = 0; // moved to top
    ...
    void setupdemoplayback()
    {
        ...
        {
            lilswap(&hdr.version, 2);
            if(hdr.version!=DEMO_VERSION) formatstring(msg)("demo \"%s\" requires an %s version of Cube 2: Sauerbomber", file, hdr.version<DEMO_VERSION ? "older" : "newer");
            else if(hdr.protocol!=PROTOCOL_VERSION) formatstring(msg)("demo \"%s\" requires an %s version of Cube 2: Sauerbomber", file, hdr.protocol<PROTOCOL_VERSION ? "older" : "newer");
        }
        if(msg[0])
        {
            DELETEP(demoplayback);
    ...
    int checktype(int type, clientinfo *ci)
    {
        ...
        if(ci) 
        {
            ...
            {
                if(type != N_POS && type != N_EXPLODE && ++ci->overflow >= 200) return -2; // TEMPORARY FIX (suggested by upper, enhanced by pisto):
            }
        }
        return type;
    }
