...
bool loadents(const char *fname, vector<entity> &ents, uint *crc)
{
    ...
    if(hdr.version>MAPVERSION) { conoutf(CON_ERROR, "map %s requires a newer version of Cube 2: Sauerbomber", ogzname); delete f; return false; }
    compatheader chdr;
    if(hdr.version <= 28)
    {
...
bool load_world(const char *mname, const char *cname)        // still supports all map formats that have existed since the earliest cube betas!
{
    ...
    execfile("data/cubescript/defaults/default_map_settings.cfg", false);
